function Invoke-ADCAddCSPolicy {
    <#
    .SYNOPSIS
        Add a new Content-Switching Policy resource
    .DESCRIPTION
        Add a new Content-Switching Policy resource
    ...
    .EXAMPLE
        Invoke-ADCAddCSPolicy -PolicyName csp_www.domain.com_https -Rule 'HTTP.REQ.HOSTNAME.TO_LOWER.CONTAINS("www.domain.com")' -Action csa_www.domain.com_https
    .NOTES
    ...
    param(
        ...
        [String]$PolicyName = (Read-Host -Prompt "Name of the new Content Switch Policy"),
    ...
    process {
        try {
            ...
            if ($PSCmdlet.ShouldProcess($PolicyName, 'Create Content Switch Policy')) {
                $result = Invoke-ADCNitroApi -ADCSession $ADCSession -Method POST -Type cspolicy -Payload $Payload -GetWarning
                if ($PSBoundParameters.ContainsKey('PassThru')) {
                    Write-Output (Invoke-ADCGetSSLCertKey -Name $PolicyName)
                } else {
                    Write-Output $result
                }
            }
