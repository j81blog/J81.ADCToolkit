    ...
    process {
        try {
            if ( $PsCmdlet.ParameterSetName -eq 'Getall' ) {
                ...
                $response = Invoke-ADCNitroApi -ADCSession $ADCSession -Method GET -Type snmp -Query $Query -Summary:$ViewSummary -Filter $Filter -GetWarning
                ...
                $response = Invoke-ADCNitroApi -ADCSession $ADCSession -Method GET -Type snmp -Arguments $Arguments -GetWarning
            } elseif ( $PsCmdlet.ParameterSetName -eq 'GetByResource' ) {
                Write-Verbose "Retrieving snmp configuration for property ''"

            } else {
                Write-Verbose "Retrieving snmp configuration objects"
                $response = Invoke-ADCNitroApi -ADCSession $ADCSession -Method GET -Type snmp -Summary:$ViewSummary -Query $Query -Filter $Filter -GetWarning
            }
        } catch {
